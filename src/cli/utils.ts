 ...
 */
export function parseArgs(args: string[], argDefs: ArgDef[]): {
  ...
  if (actionName && module.actions[actionName]) {
    ...
    if (action.args && action.args.length > 0) {
      ...
      for (const arg of action.args) {
        ...
        lines.push(`  ${short}--${arg.name} <value>  ${arg.description} ${required}${defaultVal}`);
      }
      lines.push('');
    }
    ...
    lines.push('');
    lines.push(`Run 'affine-cli ${module.name} <action> --help' for more information on a specific action.`);
  }
