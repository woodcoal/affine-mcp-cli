			...
			if (workspacePageIds && graphEdges.length > workspacePageIds.size) {
				for (const edge of graphEdges) {
					...
					const edgeExists = Boolean(
						edgeSnapshot.missing || edgeSnapshot.state || edgeSnapshot.timestamp
					);
					if (!edgeExists) {
 ...
 */
export async function getDocHandler(parsed: { workspaceId?: string; docId: string }) {
	const gql = await createGraphQLClient();
	const workspaceId = parsed.workspaceId || getDefaultWorkspaceId();
	if (!workspaceId) {
		throw new Error(
 ...
 */
export async function deleteDocHandler(parsed: { workspaceId?: string; docId: string }) {
	const workspaceId = parsed.workspaceId || getDefaultWorkspaceId();
