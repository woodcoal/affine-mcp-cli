...
export const AppendBlockBookmarkStyle = z.enum(APPEND_BLOCK_BOOKMARK_STYLE_VALUES);
...
export const AppendBlockDataViewMode = z.enum(APPEND_BLOCK_DATA_VIEW_MODE_VALUES);
...
export function getTagArray(target: Y.Map<any>, key: string = 'tags'): Y.Array<string> | null {
...
export function ensureTagArray(target: Y.Map<any>, key: string = 'tags'): Y.Array<string> {
...
export function getWorkspaceTagOptionsArray(meta: Y.Map<any>): Y.Array<any> | null {
...
export function parseWorkspaceTagOption(raw: unknown): WorkspaceTagOption | null {
	let id: unknown;
	let value: unknown;
	let color: unknown;
...
export function deleteArrayIndexes(arr: Y.Array<any>, indexes: number[]): boolean {
	if (indexes.length === 0) {
		return false;
	}
...
export function hasTag(tagValues: string[], tag: string, ignoreCase: boolean): boolean {
	const normalizedTag = ignoreCase ? tag.toLocaleLowerCase() : tag;
	return tagValues.some(
		(entry) => (ignoreCase ? entry.toLocaleLowerCase() : entry) === normalizedTag
	);
}
...
export function getWorkspacePageEntries(meta: Y.Map<any>): WorkspacePageEntry[] {
...
export function collectDescendantBlockIds(blocks: Y.Map<any>, startIds: string[]): string[] {
	const result: string[] = [];
...
export function extractTableData(block: Y.Map<any>): string[][] | null {
	...
	for (const { rowId } of rowEntries) {
		const row: string[] = [];
		for (const { columnId } of columnEntries) row.push(cells.get(`${rowId}:${columnId}`) ?? '');
		tableData.push(row);
	}
	return tableData;
}
...
export async function createDocInternal(parsed: CreateDocInput): Promise<CreateDocResult> {
...
export function buildDatabaseColumnLookup(columnDefs: DatabaseColumnDef[]): DatabaseColumnLookup {
...
export function ensureDatabaseRowCells(cellsMap: Y.Map<any>, rowBlockId: string): Y.Map<any> {
...
export function resolveSelectOptionId(
	...
	if (!createOption) throw new Error(`Column "${col.name}": option "${valueText}" not found`);
...
export function decodeDatabaseCellValue(
	...
	const rawValue =
		cellEntry instanceof Y.Map ? cellEntry.get('value') : (cellEntry as any)?.value;
	...
	switch (col.type) {
		...
		case 'multi-select': {
			...
			const values = optionIds.map(
				(optionId) => col.options.find((entry) => entry.id === optionId)?.value ?? optionId
			);
