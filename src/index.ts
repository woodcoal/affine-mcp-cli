...
console.error(
  `[affine-mcp] Auth: ${hasAuth ? "configured" : "not configured"}`,
);
...
if (
  ...
  console.error(
    "WARNING: Credentials configured over plain HTTP. Use HTTPS for remote servers.",
  );
}
console.error(
  `[affine-mcp] Workspace: ${config.defaultWorkspaceId ? "set" : "(none)"}`,
);
...
if (config.authMode === "oauth" && !useHttpTransport) {
  throw new Error(
    "AFFINE_MCP_AUTH_MODE=oauth requires MCP_TRANSPORT=http (or streamable/sse).",
  );
}
...
async function buildServer() {
  ...
  if (config.authMode === "oauth") {
    if (!gqlBearer) {
      throw new Error(
        "AFFINE_API_TOKEN is required when AFFINE_MCP_AUTH_MODE=oauth.",
      );
    }
    ...
    if (mode === "sync") {
      ...
      (async () => {
        try {
          ...
          gql.setCookie(cookieHeader);
          console.error(
            "Successfully authenticated with email/password (async)",
          );
        } catch (e) {
          console.error(
            "Failed to authenticate with email/password (async):",
            e,
          );
        }
      })();
  ...
  if (!gql.isAuthenticated()) {
    console.error(
      "WARNING: No authentication configured. Some operations may fail.",
    );
  ...
  console.error(
    `[affine-mcp] Disabled tools: ${process.env.AFFINE_DISABLED_TOOLS || "(none)"}`,
  );
