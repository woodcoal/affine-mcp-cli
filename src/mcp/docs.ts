...
import * as crud from '../core/docs/crud.js';
import * as search from '../core/docs/search.js';
import * as tags from '../core/docs/tags.js';
import * as blocks from '../core/docs/blocks.js';
import * as markdown from '../core/docs/markdown.js';
import * as database from '../core/docs/database.js';
import * as publish from '../core/docs/publish.js';
...
export function registerDocTools(server: McpServer) {
	...
	server.registerTool(
		'duplicate_doc',
		{
			...
			description: 'Clone a document by copying its markdown content into a new doc.',
			inputSchema: {
				workspaceId: z.string().optional(),
				docId: z.string(),
				title: z.string().optional(),
	...
	server.registerTool(
		'create_doc_from_template',
		{
			...
			description: 'Clone a template doc and substitute {{variable}} placeholders.',
			inputSchema: {
				workspaceId: z.string().optional(),
	...
	server.registerTool(
		'get_doc_by_title',
		{
			...
			description: 'Find a document by title and return its content as markdown.',
			inputSchema: {
				workspaceId: z.string().optional(),
				query: z.string(),
				limit: z.number().optional()
	...
	server.registerTool(
		'move_doc',
		{
			...
			description: 'Move a doc in the sidebar by embedding it under a new parent.',
			inputSchema: {
				workspaceId: z.string().optional(),
				docId: z.string(),
	...
	server.registerTool(
		'find_and_replace',
		{
			...
			description: 'Find and replace text across all Y.Text fields in a document.',
			inputSchema: {
				workspaceId: z.string().optional(),
				docId: z.string(),
	...
	server.registerTool(
		'update_database_row',
		{
			...
			description: 'Batch update multiple cells on an existing AFFiNE database row.',
			inputSchema: {
				workspaceId: z.string().optional(),
				docId: DocId,
				databaseBlockId: z.string().min(1),
