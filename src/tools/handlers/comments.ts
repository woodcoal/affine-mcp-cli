...
import { wsUrlFromGraphQLEndpoint, connectWorkspaceSocket, joinWorkspace, loadDoc } from "../../ws.js";
 ...
 * 列出评论
 */
export async function listCommentsHandler(params: ListCommentsParams) {
  ...
  const data = await gql.request<{ workspace: any }>(query, { workspaceId, docId, first, offset, after });
 ...
 * 创建评论
 */
export async function createCommentHandler(params: CreateCommentParams) {
  const gql = getGraphQLClient();
  const workspaceId = params.workspaceId || getDefaultWorkspaceId();
  if (!workspaceId) throw new Error("workspaceId required (or set AFFINE_WORKSPACE_ID)");
  ...
  const normalizedDocMode = (params.docMode || 'page').toLowerCase() === 'edgeless' ? 'edgeless' : 'page';
  const normalizedContent = typeof params.content === 'string' ? { text: params.content } : params.content;
  const input = { content: normalizedContent, docId: params.docId, workspaceId, docTitle: params.docTitle || "", docMode: normalizedDocMode, mentions: params.mentions };
 ...
 * 更新评论
 */
export async function updateCommentHandler(params: UpdateCommentParams) {
  const gql = getGraphQLClient();
  const mutation = `mutation UpdateComment($input: CommentUpdateInput!){ updateComment(input:$input) }`;
  const data = await gql.request<{ updateComment: boolean }>(mutation, { input: { id: params.id, content: params.content } });
 ...
 * 删除评论
 */
export async function deleteCommentHandler(params: DeleteCommentParams) {
  const gql = getGraphQLClient();
  const mutation = `mutation DeleteComment($id:String!){ deleteComment(id:$id) }`;
  const data = await gql.request<{ deleteComment: boolean }>(mutation, { id: params.id });
 ...
 * 解决评论
 */
export async function resolveCommentHandler(params: ResolveCommentParams) {
  const gql = getGraphQLClient();
  const mutation = `mutation ResolveComment($input: CommentResolveInput!){ resolveComment(input:$input) }`;
  const data = await gql.request<{ resolveComment: boolean }>(mutation, { input: params });
