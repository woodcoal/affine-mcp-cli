...
export async function listDocsByTagHandler(parsed: {
  ...
  try {
    ...
    const docs = pages
      ...
      .filter(
        (page) =>
          hasTag(page.tags, tag, ignoreCase) ||
          hasTag(page.rawTags, tag, ignoreCase),
      )
 ...
 */
export async function createTagHandler(parsed: {
  workspaceId?: string;
  tag: string;
}) {
  const workspaceId = parsed.workspaceId || getDefaultWorkspaceId();
  ...
  try {
    ...
    if (!snapshot.missing) {
      throw new Error(
        `Workspace root document not found for workspace ${workspaceId}`,
      );
    }
...
export async function addTagToDocHandler(parsed: {
  ...
  try {
    ...
    const page = getWorkspacePageEntries(wsMeta).find(
      (entry) => entry.id === parsed.docId,
    );
    if (!page) {
      throw new Error(
        `docId ${parsed.docId} is not present in workspace ${workspaceId}`,
      );
    }

    const { option, created: optionCreated } = ensureWorkspaceTagOption(
      wsMeta,
      tag,
    );
...
export async function removeTagFromDocHandler(parsed: {
  ...
  try {
    ...
    const option =
      getWorkspaceTagOptionMaps(wsMeta).byValueLower.get(
        tag.toLocaleLowerCase(),
      ) || null;
    const pageTags = ensureTagArray(page.entry);
    const pageTagIndexes = collectMatchingTagIndexes(
      pageTags,
      tag,
      option,
      true,
    ...
    if (!docSnapshot.missing) {
      ...
      const docMeta = doc.getMap("meta");
      const docTags = ensureTagArray(docMeta);
      const docTagIndexes = collectMatchingTagIndexes(
        docTags,
        tag,
        option,
        true,
 ...
 */
export async function getDocsByTagHandler(parsed: {
  workspaceId?: string;
  tag: string;
}) {
  const workspaceId = parsed.workspaceId || getDefaultWorkspaceId();
  ...
  try {
    ...
    const matched = pages
      ...
      .filter(({ rawTagIds }) =>
        rawTagIds.some((tid) => matchingTagIds.has(tid)),
      )
      .map(({ p, rawTagIds }) => ({
        docId: p.id,
