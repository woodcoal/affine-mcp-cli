 ...
 * 列出文档历史记录的参数类型
 */
export interface ListHistoriesParams {
  workspaceId?: string;
  guid: string;
  take?: number;
  before?: string;
 ...
 * 列出文档历史记录（时间戳）
 */
export async function listHistoriesHandler(params: ListHistoriesParams) {
  ...
  const data = await gql.request<{ workspace: any }>(query, { workspaceId, guid, take, before });
